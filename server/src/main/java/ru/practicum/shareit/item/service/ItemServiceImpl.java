...
public class ItemServiceImpl implements ItemService {
    ...
    @Transactional
    @Override
    public ItemDto addNewItem(AddNewItemRequest item, Integer userId) {
        ...
        if (item.getRequestId() == null) {
            Item itemWithoutRequest = ItemMapper.toItem(item);

            return ItemMapper.toItemDto(itemRepository.save(itemWithoutRequest));
        } else {
            Request request = requestRepository.findById(item.getRequestId()).get();
            Item itemWithRequest = ItemMapper.toItem(item);
            itemWithRequest.setRequest(request);

            return ItemMapper.toItemDto(itemRepository.save(itemWithRequest));
        }
    }
